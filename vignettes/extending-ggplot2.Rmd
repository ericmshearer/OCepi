...
-   `end_points` - subset at last time series data point
...
For line plots with >1 group, it is recommended to direct label groups. For line plots, `end_points` will subset the data to the last date in a time series, even if data contains >1 group and those groups end at different dates (thank you Butte County for the suggestion).
...
In plots with multiple categories, you may want to draw attention to a specific group - whether it's the highest/lowest value, a group of interest, etc. `highlight_geom` requires two main arguments - 1) an expression (similar to what you'd use in `dplyr::filter`), and 2) a color for highlighting. Although sensible defaults are built-in, the following arguments can be modified:

-   alpha: transparency
-   linewidth: line thickness (`geom_line`, `geom_sf`)
-   size: point size

An alternative to `highlight_geom` is `desaturate_geom`. Instead of fading non-emphasized categories to gray, they will retain color but desaturated to a specified level using argument `desaturate`. Options to customize include:

-   desaturate: level of desaturation ranging from 0 to 1 (with 1 being highest level of desaturation)
-   size: point size
-   linewidth: line thickness (`geom_line`, `geom_sf`)
...
ggplot(data = ts, aes(x = date, y = scores, group = site, label = site)) +
  geom_line(linewidth = 1) +
  ...
  apollo_label(data = end_points(ts, date), hjust = -0.3)
