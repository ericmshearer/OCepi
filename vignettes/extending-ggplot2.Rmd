...
The highlighting does work with `facet_wrap` and `facet_grid`. Currently works with `geom_col`/`geom_bar`, `geom_line`, `geom_sf`, and `geom_point`.
...
ggplot(data = points, aes(x = x, y = y)) +
  geom_point(size = 3) +
  highlight_geom(y %in% c(20:25), pal = "darkred", size = 4) +
  scale_y_continuous(expand = c(0,0), limits = c(0,55)) +
  theme_apollo(direction = "vertical") +
  labs(
