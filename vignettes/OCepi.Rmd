...
For categories with long titles (e.g. race/ethnicity), it may work best to wrap text to better fit within plot. Use `width` argument to adjust amount of wrapping (lower width = more wrapping, higher width = less wrapping)
.
Without wrapping:

```{r no-wrap, fig.height=7, fig.width=10}
...
```{r wrap-label, fig.height=7, fig.height=10}
ggplot(data = df, aes(x = group, y = score)) +
  geom_col() +
...
```{r cleveland, fig.height=8, fig.width=10}
